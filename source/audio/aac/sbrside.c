...
static void UnpackSBRGrid(BitStreamInfo *bsi, SBRHeader *sbrHdr, SBRGrid *sbrGrid) {
    ...
    case SBR_GRID_FIXFIX:
        ...
        /*  SKYWAVE: clamp numEnv. numEnvRaw is 2 bits straight off the wire, so
            (1 << numEnvRaw) reaches 8, but every array it indexes is sized
            MAX_NUM_ENV (5): freqRes[] just below overruns the struct, and
            relBordLead[] further down overruns the STACK. The ASSERT beneath
            states the real contract but expands to nothing on GCC, so it stopped
            nothing. Measured with ASan on a crafted grid (frameClass=FIXFIX,
            numEnvRaw=3): "stack-buffer-overflow ... WRITE of size 1" at the
            relBordLead store, plus UBSan "index 5 out of bounds" at freqRes.
            Clamped to 4 rather than to MAX_NUM_ENV because the border
            arithmetic below divides NUM_TIME_SLOTS by numEnv and takes
            numEnv >> 1, both of which assume a power of two; 5 would be in
            bounds but wrong. A frame asking for 8 envelopes is outside what
            this decoder supports either way, so it mis-decodes instead of
            corrupting memory. The other frame classes already carry equivalent
            "&& env < MAX_NUM_ENV" guards - FIXFIX was the one without any.  */
        if (sbrGrid->numEnv > 4) {
            sbrGrid->numEnv = 4;
        }
        ...
        /*  SKYWAVE: added "&& env < MAX_NUM_ENV", matching the guard the VARFIX
            and VARVAR cases already carry. Redundant now that numEnv is clamped
            above, but not merely cosmetic: freqRes[] sits next to numEnv in the
            struct, so while this store was unbounded the compiler could not rule
            out its clobbering numEnv, lost the range it had just proved, and
            went on warning that the relBordLead[] store below was unbounded
            too. The explicit guard is what lets that be proved.  */
        for (env = 1; env < sbrGrid->numEnv && env < MAX_NUM_ENV; env++) {
        ...
        /*  SKYWAVE: added "&& rel < MAX_NUM_ENV". numRelLead is numEnv - 1, so
            the clamp above already bounds it at 3 - this is the belt to that
            braces, and it is the store ASan actually caught overflowing.  */
    ...
    case SBR_GRID_VARVAR:
        ...
        /*  SKYWAVE: clamp numEnv here too. Both counts are 2 bits off the wire,
            so this reaches 3 + 3 + 1 = 7 against MAX_NUM_ENV of 5. The freqRes
            and relBorder loops in this case already carry their own bounds
            guards, so nothing overruns inside THIS function - but numEnv is
            written into the grid and every consumer downstream indexes
            envTimeBorder[MAX_NUM_ENV + 1] and envDataQuant[MAX_NUM_ENV] by it.
            The ASSERT below already states the bound and already expands to
            nothing on GCC. Clamping to MAX_NUM_ENV is exact here: unlike FIXFIX
            above, nothing in this case requires a power of two.  */
        if (sbrGrid->numEnv > MAX_NUM_ENV) {
            sbrGrid->numEnv = MAX_NUM_ENV;
        }
    ...
    /*  SKYWAVE: bound middleBorder before it indexes envTimeBorder[] below.
        Every frame class derives it from `pointer`, which is read as
        cLog2[numEnv + 1] bits and is never checked against numEnv: with
        numEnv = 5 that is 3 bits, so pointer reaches 7 and FIXVAR/VARVAR's
        `numEnv + 1 - pointer` goes NEGATIVE, while VARFIX's `pointer - 1`
        overshoots the other way. envTimeBorder is MAX_NUM_ENV + 1 = 6 entries
        and is only filled in [0, numEnv], so both ends are out of bounds.
        Found by the exhaustive prefix sweep in tests/test_sbrgrid.c, not by
        looking for it: UBSan reported "index -1 out of bounds for type
        'unsigned char [6]'" at the noiseTimeBorder[1] assignment.
        Clamped rather than rejected to stay consistent with the numEnv clamps
        above - a frame with a nonsensical pointer mis-decodes instead of
        reading off the end of the struct.  */
    if (middleBorder < 0) {
        middleBorder = 0;
    } else if (middleBorder > sbrGrid->numEnv) {
        middleBorder = sbrGrid->numEnv;
    }
