...
/*  toolchain:           MSFT Visual C++
    ...
    SKYWAVE: added `&& !defined(__GNUC__)` below. Without it this branch also
    catches GCC-on-Windows (MinGW/MinGW-w64, this project's own host-test
    compiler - see tests/Makefile's HOSTCC) because MinGW defines _WIN32 too,
    and GCC does not understand the MSVC `__asm { mov eax, x ... }` block
    syntax a few lines down - it is a hard compile error, not a warning.
    Excluding __GNUC__ here sends MinGW down to the existing, already-portable
    `__GNUC__ && (__i386__ || __amd64__)` branch further down this file
    instead, which every other GCC target (including this project's real
    target, devkitARM/__arm__) already uses. No ARM assembly is touched by
    this change - see the separate __3DS__ addition below for that branch.
*/
#if (defined (_WIN32) && !defined (_WIN32_WCE) && !defined(__GNUC__)) || (defined (__WINS__) && defined (_SYMBIAN)) || (defined (WINCE_EMULATOR)) || (defined (_OPENWAVE_SIMULATOR))
...
/*  SKYWAVE: added the trailing `|| (... __3DS__)` clause below to route
    devkitARM into this existing portable-C branch. __3DS__ is defined by
    this project's own Makefile (-D__3DS__), so this only changes behaviour
    for this one target; every other platform's branch is untouched. This
    is not a new code path - it is the same generic Word64/MULSHIFT32 C
    implementation already used here for MIPS/PowerPC/Arduino-Xtensa, which
    is why it is safe: the real ARM inline-assembly branch above this one
    is already permanently disabled upstream (its condition tests the
    never-defined macro `XXXX__arm__`), so devkitARM was falling through to
    `#error Unsupported platform in assembly.h` with nothing to catch it.
    See source/audio/aac/README.md, "Local changes". */
